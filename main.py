...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("funcionando")
        await self.highrise.walk_to(Position(17.5 , 7.5 , 17.5 , "FrontRight"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala")   
        await self.highrise.chat(f"😍𝑯𝒆𝒚😍{user.username} ❤️𝑩𝑰𝑬𝑵𝑽𝑬𝑵𝑼𝑬 𝑨𝑼 𝑩𝑬𝑺𝑻𝑰𝑬 𝑪𝑳𝑼𝑩 𝑷𝑨𝑺𝑺𝑬 𝑫𝑬 𝑩𝑶𝑵 𝑴𝑶𝑴𝑬𝑵𝑻❤️.Pour les danses utilise de 1 à 95")
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if                            message.startswith("Summon") or          message.startswith("/summon") or  message.startswith("!summon"):
          if user.username == "daniel_offset" or user.username == "Silencieuxx" or user.username == "Demic._" or user.username == "fanny_stone" or user.username == "f__7oo" or user.username == "H.kakashi" or user.username == "Just_xx":
           target_username = message.split("@")[-1].strip()
           await self.teleport_user_next_to(target_username, user)
    
        if message.startswith("kick"):
          if user.username == "daniel_offser" or user.username == "Silencieuxx" or user.username == "Demic._" or user.username == "Mini_sad" or user.username == "f__7oo" or user.username == "H.kakashi" or user.username == "Just_xx":
              pass
          else:
